...
function start() {
    if(cm.getMapId()==60000){
        cm.sendYesNo("Take this ship and you'll head off to a bigger continent. For #e150 mesos#n, I'll take you to #bVictoria Island#k. What do you think? Do you want to go to Victoria Island?");
    }
    else if(cm.getMapId()==104000000){
        cm.sendYesNo("I can take you back to Maple Island. I would charge #e150 mesos#n for that. What do you think? Do you want to go back to Maple Island?");
    }
    else{
        cm.sendSimple("Hi, I'm captain Shanks.");
        cm.dispose();
    }
}
...
function action(mode, type, selection) {
    if(cm.getMapId()==60000){
        ...
        if (status == 1){
            cm.sendNext("Bored of this place? Alright... Give me #e150 mesos#n first...");
        } else if (status == 2) {
            if (cm.getMeso() < 150) {
                cm.sendOk("What? You're telling me you wanted to go without any money? You're one weirdo...");
                cm.dispose();
            } else {
                cm.sendNext("Awesome! #e150#n mesos accepted! Alright, off to Maple Island!");
            }
        } else if (status == 3) {
            cm.gainMeso(-150);
            cm.warp(60000, 0);
